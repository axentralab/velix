  ...
  if (!user || user.email !== ADMIN_EMAIL) {
    return (
      <div className="mx-auto max-w-4xl px-6 py-20 text-center">
        ...
        <p className="text-slate-400 mb-8">Please sign in with the admin account to access settings.</p>
    ...
    <div className="mx-auto max-w-7xl space-y-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <h1 className="text-4xl font-semibold text-white">Admin Settings</h1>
        </div>
        <button className="inline-flex items-center gap-2 rounded-full bg-slate-700 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-slate-950/20 hover:bg-slate-600">
          <FiSettings size={18} /> Manage Permissions
        </button>
      </div>

      <div className="grid gap-6 xl:grid-cols-3">
        {['Role Permissions', 'Audit Logs', 'Security Policies', 'Integrations', 'Billing'].map((item) => (
          <div key={item} className="rounded-3xl border border-slate-800 bg-slate-900 p-6 shadow-xl shadow-slate-950/40">
            ...
            <p className="mt-3 text-sm leading-6 text-slate-400">Control admin roles, security settings, and integrations for your OMS platform.</p>
          </div>
        ))}
      </div>
    </div>
